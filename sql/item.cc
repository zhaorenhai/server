...
Item::Item(THD *thd):
  name(null_clex_str), orig_name(0),
  item_flags(item_flags_t::FIXED | item_flags_t::IS_AUTOGENERATED_NAME),
...
bool Item_sp_variable::fix_fields_from_item(THD *thd, Item **, const Item *it)
{
  ...
  if (thd->lex->current_select && thd->lex->current_select->master_unit()->item)
    thd->lex->current_select->master_unit()->item->set_with_param();
  ...
  Item_result_field(thd), orig_item(item_arg), expr_cache(NULL), expr_value(NULL)
{
  ...
  copy_maybe_null(orig_item);
  copy_with_flags(orig_item);
  name= item_arg->name;
  set_fixed();
